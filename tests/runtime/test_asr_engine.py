...
class TestDistilWhisperModel:
    ...
    def test_quick_tier_uses_cpu_low_latency_model(self):
        ...
        engine = ASREngine(tier=ModelTier.QUICK, device="cpu", compute_type="int8")
        assert engine.model_config.model_id == "small.en"

    def test_quick_tier_resolver_cuda_prefers_distil(self):
        """Test quick tier resolver keeps distil on CUDA"""
        from voiceflow.core.asr_engine import ModelTier, resolve_tier_model_key

        assert resolve_tier_model_key(ModelTier.QUICK, "cuda") == "distil-large-v3"
