...
class _NullWhisperModel:
    """Null object fallback when Whisper model is unavailable."""
    ...
    def transcribe(self, audio, **kwargs):
        # Provide empty transcription result to keep pipeline stable
        return [], {"text": "", "null_model": True}
...
class BufferSafeWhisperASR:
    ...
    def __init__(self, cfg: Config):
        self.cfg = cfg
        self._model = None
        self._null_model = _NullWhisperModel()
        ...
        self._buffer_protector = BufferOverflowProtection()
    ...
    def load(self):
        ...
        if model is None:
            raise RuntimeError(
                "Failed to load Whisper model. Ensure faster-whisper is installed."
            )
    ...
    def _create_fresh_model(self):
        """Create a new Whisper model instance without disturbing the current one."""
        try:
            from faster_whisper import WhisperModel
        except Exception as import_error:
            logger.error(f'faster-whisper import failed: {import_error}')
            return None
        ...
        try:
            model = WhisperModel(
                ...
                compute_type=getattr(self.cfg, 'compute_type', 'int8'),
            )

            # Minimal warmup so the first real transcription is responsive
            try:
                silence = np.zeros(1600, dtype=np.float32)
                segments, _info = model.transcribe(
                    silence,
                    language=getattr(self.cfg, 'language', None),
                    vad_filter=False,
                    beam_size=1,
                    temperature=0.0,
                ...
                logger.debug(f'Warmup skipped due to: {warmup_error}')

            return model
        except Exception as creation_error:
            logger.error(f'Failed to create fresh Whisper model: {creation_error}')
            return None

    def _get_model_or_fallback(self):
        """Return the active model or a safe fallback."""
        model = self._model
        if model is None:
    ...
    def _ensure_model_available(self):
        """Ensure a usable model instance is available and return it."""
        if self._model is None:
            try:
                self.load()
            except Exception as load_error:
                logger.error(f'Whisper model load failed: {load_error}')

        return self._get_model_or_fallback()

    def _cleanup_model(self, model):
        """Best-effort cleanup for an obsolete Whisper model."""
        if not model:
            return
        ...
        try:
            ...
            logger.debug(f'Error while closing Whisper model: {close_error}')
    ...
    def _reload_model_fresh(self) -> bool:
        """Reload the Whisper model without dropping the existing instance prematurely."""
        logger.info('Reloading Whisper model with preserve-then-replace strategy')
        ...
        if new_model is None:
            logger.error('Failed to create a fresh Whisper model; keeping existing instance')
            return False
    ...
    def transcribe(self, audio: np.ndarray) -> str:
        ...
        try:
            ...
            # CRITICAL: Buffer validation failure short-circuit to idle
            if "Buffer validation failed" in str(e) or "Audio failed ASR-specific validation" in str(e):
                logger.warning("Buffer validation failed - short-circuiting to idle to prevent re-pooling stale audio")
                # Don't increment error counters for buffer validation failures
                self._is_processing = False
                return ""
    ...
    def _process_chunked_audio(self, audio: np.ndarray, recording_state: dict) -> str:
        ...
        for i in range(0, len(audio), self._chunk_size_samples - overlap_samples):
            ...
            # Create chunk recording state with proper metadata rebuild
            ...
            # CRITICAL: Rebuild metadata for chunk to fix integrity checks
            chunk_state['audio_metadata'] = {
                'original_length': len(chunk),
                'sanitized_length': len(chunk),
                'corruption_removed': False,
    ...
    def _create_clean_recording_state(self, audio: np.ndarray) -> dict:
        ...
        try:
            ...
            # CRITICAL: Buffer overflow protection validation
            is_valid, error_msg = self._buffer_protector.validate_audio_buffer(sanitized_audio)
            if not is_valid:
    ...
    def _perform_isolated_transcription(self, recording_state: dict) -> str:
        ...
        try:
            ...
            model_instance = self._ensure_model_available()

            if model_instance is self._null_model:
                logger.error('Whisper model unavailable after recovery attempts; returning empty transcription')
                self._is_processing = False
            ...
            # CRITICAL: Apply buffer protection cleaning before returning
            if text:
                # Clean transcription and detect hallucinations
                cleaned_text = self._buffer_protector.clean_transcription(text)
                cleaned_text = self._buffer_protector.detect_trailing_garbage(cleaned_text)

                # CRITICAL: Drop "okay okay okay" hallucinations if entire text is hallucination
                if self._buffer_protector.detect_okay_hallucination(cleaned_text):
                    logger.warning(f"Detected 'okay' hallucination, dropping transcript: '{cleaned_text[:50]}...'")
                    return ""

                # Additional aggressive cleanup for trailing "okay" patterns
                # CRITICAL: Only apply if we have substantial text to avoid breaking empty results
                if len(cleaned_text.strip()) > 10:  # Only process if we have real content
                    import re
                    # Pattern to match trailing "okay" repetitions with various punctuation
                    trailing_okay_pattern = r'[\.\!\?]*\s*(?:okay[.\!\?]*\s*){2,}$'
                    before_regex = cleaned_text
                    cleaned_text = re.sub(trailing_okay_pattern, '.', cleaned_text, flags=re.IGNORECASE)

                    # Also check for pattern where okay appears after the last meaningful word
                    if len(cleaned_text) > 20:
                        # Split by common punctuation and check last segments
                        parts = re.split(r'([.!?])', cleaned_text)
                        if len(parts) >= 2:
                            last_part = parts[-1].strip().lower()
                            if re.match(r'^(?:okay\s*){2,}$', last_part):
                                # Remove the trailing okays
                                cleaned_text = ''.join(parts[:-1]).strip()

                    # If we cleaned everything away, restore original
                    if not cleaned_text.strip() and before_regex.strip():
                        cleaned_text = before_regex
